...
SRC="$DIR/data/voix/clement_voice.wav"
if [ "${1:-}" = "-a" ] || [ "${1:-}" = "--audio" ]; then
...
OUT="$DIR/data/clones/clone_$STAMP.wav"
mkdir -p "$DIR/data/clones"

# 1) Conversion WAV 16 kHz mono (VoxCPM ne sait pas lire m4a/mp3)
WAV="$DIR/data/clones/src_$STAMP.wav"
...
HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 "$DIR/.venv-whisper/bin/python" - "$WAV" > "$DIR/data/clones/transcript_$STAMP.txt" <<'EOF'
import sys
...
EOF
echo ">> transcript : $(cat "$DIR/data/clones/transcript_$STAMP.txt")"
...
./.venv/bin/voxcpm clone --local-files-only \
  --text "$TEXT" \
  --prompt-audio "$WAV" \
  --prompt-file "$DIR/data/clones/transcript_$STAMP.txt" \
  --reference-audio "$WAV" \
